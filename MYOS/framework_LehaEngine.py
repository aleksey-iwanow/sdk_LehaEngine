...
class GameObject:
    ...
    def set_index(self, index_):
        self.window.list_objects += [self.window.list_objects.pop(index_)]
    ...
    def draw(self, win):
        if self.parent:
            self.activeself = self.parent.activeself
...
class Text:
    def __init__(self, text, sizef, colorf):
        ...
        self.font = leha.font.Font("fonts/CatV.ttf", self.sizef)
    ...
    def set_font(self, font):
        self.font = leha.font.Font(font, self.sizef)
...
class Particle:
    ...
    def draw(self, win):
        win.blit(self.gm, (round(self.rect_position[0] * self.gm.window.scale_factor), round(self.rect_position[1] * self.gm.window.scale_factor)))
...
class Window:
    ...
    def active_panel(self, name):
        gm = self.find_object_of_name(name)
        gm.activeself = False if gm.activeself else True
