...
class GameObject:
    ...
    def draw(self, win):
        if self.activeself:
            ...
            if self.path != 'None' and not self.ColorBox:
                self.image = leha.transform.scale(self.sprite_copy, (self.size[0], self.size[1]))
                image2 = leha.transform.rotate(self.sprite_copy, self.angle)
    ...
    def move(self, vector, speed, power=1, is_time=True, type_exceptions=None):
        ...
        if current_time >= self.end_time or not is_time:
            ...
            if self.name == 'camera-main':
                for p in self.window.list_objects_in_camera:
                    if (p.Collider or p.rect) and not p.collision(vc=[-vc[0], -vc[1]], type_exceptions=type_exceptions):
                        self.pos += vc
            else:
                if (self.Collider or self.rect) and not self.collision(vc=[vc[0], vc[1]], type_exceptions=type_exceptions):
                    self.pos += vc
    ...
    def collision_left(self):
        ...
        objs = self.window.get_collision_objects()
        for o in objs:
            if o.name != self.name and (o.rect.collidepoint((self.rect.center[0] - self.rect.width / 2,
                                                            self.rect.center[1] - self.rect.height / 2)) or
                                        o.rect.collidepoint((self.rect.center[0] - self.rect.width / 2,
...
class Button:
    ...
    def click(self):
        if self.gameObject.activeself:
            ...
            rect = leha.Rect((self.gameObject.rectpos[0] - self.gameObject.size[0] // 2,
                              self.gameObject.rectpos[1] - self.gameObject.size[1] // 2, self.gameObject.size[0],
                              self.gameObject.size[1]))
            if rect.collidepoint(leha.mouse.get_pos()[0], leha.mouse.get_pos()[1]):
                if self.color_loc != 'None' and self.start_color:
                    ...
                    (cl[0] - 15) if cl[0] >= 10 else 0, (cl[1] - 15) if cl[1] >= 15 else 0,
                    (cl[2] - 15) if cl[2] >= 15 else 0)
...
class Particle:
    ...
    def draw(self, win):
        win.blit(self.gm, (self.rect_position[0], self.rect_position[1]))
...
class UI:
    def __init__(self, window, name, image_path, pos, size, angle, opacity, text, color_, pad, sz_font, color_font, is_button):
        self.window = window
        self.name = name
        self.path = image_path
        self.pos = pos
        ...
        self.color = color_
        if color_ != 'None':
            self.color = leha.Color(color_)
            self.start_color = leha.Color(color_)
        self.pad = pad
        self.font_sz = sz_font
        self.font_cl = color_font
        self.is_button = is_button
        if self.is_button:
            self.functions = []

        if image_path != 'None':
            self.image = leha.image.load(window.path_delta + self.path if window.b else self.path)
            self.image.set_alpha(255 * opacity)
            self.image = leha.transform.scale(self.image, (size[0], size[1]))
            self.image = leha.transform.rotate(self.image, angle)
            self.sprite_copy = self.image
            self.rect = self.image.get_rect(center=(pos[0], pos[1]))

    def set_functions(self, func):
        self.functions = func

    def is_click(self):
        if self.color != 'None':
            self.color.r = self.start_color.r
            self.color.g = self.start_color.g
            self.color.b = self.start_color.b

        rect = leha.Rect((self.pos[0] - self.size[0] // 2, self.pos[1] - self.size[1] // 2, self.size[0], self.size[1]))
        if rect.collidepoint(leha.mouse.get_pos()[0], leha.mouse.get_pos()[1]):
            if self.color != 'None':
                cl = self.start_color
                self.color.r = (cl.r - 20) if cl.r >= 20 else 0
                self.color.g = (cl.g - 20) if cl.g >= 20 else 0
                self.color.b = (cl.b - 20) if cl.b >= 20 else 0
            if leha.mouse.get_pressed()[0]:
                return True
    ...
    def draw(self, win):
        if self.activeself:
            if self.path != 'None':
                ...
                cl = self.color
                s = leha.Surface((self.size[0], self.size[1]), leha.SRCALPHA)
                s.fill((cl.r, cl.g, cl.b, int(255 * self.opacity)))
                win.blit(s, (self.pos[0] - self.size[0] / 2, self.pos[1] - self.size[1] / 2))
            if self.text != "None":
                cl2 = leha.Color(self.font_cl)
                text_ = self.font.render(self.text, True, cl2)
                if self.is_center:
                    text_rect = text_.get_rect(center=(self.pos[0], self.pos[1]))
                    win.blit(text_, text_rect)
                else:
                    win.blit(text_, (self.pos[0] - self.size[0] / 2 + self.pad[0], self.pos[1] - self.size[1] / 2 + self.pad[1]))
...
class Window:
    ...
    def copy(self, gameObject, pos=None, components=None):
        return self.create_object(gameObject.name, gameObject.path, [pos[0], pos[1]] if pos else gameObject.pos, gameObject.size, gameObject.angle, gameObject.opacity, gameObject.is_collision, gameObject.add_to_list, components if components else gameObject.components, self.list_objects.index(gameObject) + 1)
                ...
                if split_[1].strip() != '':
                    for i in sp2:
                        ...
                        ui = UI(self,
                                ...
                                objparam[6],
                                objparam[7],
                                [int(a) for a in objparam[8].split(',')],
                                int(objparam[9]),
                                objparam[10],
                                booled(eval(objparam[11])))

                        list_ui.append(ui)
        return [list_objects, list_debug_objects, list_ui]
            ...
            if gameObject.name != 'anim_':
                try:
                    if gameObject not in engine.list_objects_in_camera and (not gameObject.UI or (gameObject.UI and not gameObject.UI.active)):
                        gameObject.rectpos = [gameObject.pos[0] + engine.camera.pos[0], gameObject.pos[1] + engine.camera.pos[1]]
                        gameObject.rect.center = gameObject.rectpos
                    if gameObject.parent:
                        gameObject.rectpos = [gameObject.pos[0] + gameObject.parent.rectpos[0], gameObject.pos[1] + gameObject.parent.rectpos[1]]
                        gameObject.rect.center = gameObject.rectpos
...
def get_func_info(class_, name_):
    index_ = 1
    if class_ == 'Window':
        tx = getsource(getattr(Window, name_))
        tx_sp = tx.split('\n')
        txx = ''
        for i in tx_sp:
            txx += f'{(len(tx_sp) // 10 - index_ // 10) * " "}{index_}| {i[4:]}\n'
            index_ += 1

        return txx
    elif class_ == 'GameObject':
        tx = getsource(getattr(GameObject, name_))
        tx_sp = tx.split('\n')
        txx = ''
        for i in tx_sp:
            txx += f'{(len(tx_sp) // 10 - index_ // 10) * " "}{index_}| {i[4:]}\n'
        ...
        if name_ in FUNCTIONS:
            tx = getsource(FUNCTIONS[name_])
            tx_sp = tx.split('\n')
            txx = ''
            for i in tx_sp:
                txx += f'{(len(tx_sp) // 10 - index_ // 10) * " "}{index_}| {i}\n'
                index_ += 1
            return txx
        else:
...
def quit_app():
    with open(f'fps.settings', 'w') as fl:
        for i in FPS_LS:
